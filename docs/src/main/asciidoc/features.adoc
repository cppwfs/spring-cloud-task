...
[[enable-task-metrics]]
=== Enable Task Metrics
Spring Cloud Task integrates with Micrometer and collects metrics for both Observable and LongTaskTimer.
To enable Task metrics integration, you must add the spring-boot-starter-actuator,
preferred registry implementation, and the micrometer core to your task application.
An example maven set of dependencies to enable task metrics using Influx would be:
